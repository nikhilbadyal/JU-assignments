...
void display(unsigned int  t,sem_t* p2_sw, sem_t* p2_sp ){
	int i;
	for(i = 1; i <= 10; i++){
		sem_wait(p2_sw);//To lock a semaphore or wait we can use the sem_wait function
		printf("Parent: %d, Process: %d, Itr: %d\n",getppid(),getpid(),i);
		sem_post(p2_sp);//To release or signal a semaphore, we use the sem_post function
		sleep(t);
	}
}
