                ...
                if (NumberOfAttempts < 15) { //15 is the maximum number of attempts
                    try {
                        if (ChannelStatus == OCCUPIED) {
                            ...
                            try {
                                Thread.sleep(100); // In Non-persistent sensing, Sender will wait for random amount of time
                            }
                            ...
                            if (ChannelStatus == FREE && messageTransmitting == 0) {//Successful transmission
                                ...
                                startThroughput = end;
                                synchronized(OnePersistence.throughput){
                                    OnePersistence.throughput.add(time);


                                    if(!OnePersistence.forwardDelay.containsKey(StationNumber)){
                                        OnePersistence.forwardDelay.put(StationNumber, new ArrayList<>());
                                    }
                                    OnePersistence.forwardDelay.get(StationNumber).add(System.currentTimeMillis()-startDelay);
