...
class OneChannel implements Runnable, ChannelConstants {
    ...
    OneChannel(String threadName, int maxFrameNumber, FileWriter fileWriter) {
    ...
    public void run() {
        ...
        while (!checkIfSuccessfulTransmission.get()) {
            ...
            while(currentFrameNumber <= maxFrameNumber) {
                OnePersistent.updateTotalCount();
                    ...
                    try {
                        if (channelStatus == INUSE) {
                            ...
                            if (channelStatus == FREE && distance == 0) {//Successful transmission
                                ...
                                synchronized (this){
                                    OnePersistent.forwarding_delay += end-start;
                                }
                                OnePersistent.updateDoneCount();
