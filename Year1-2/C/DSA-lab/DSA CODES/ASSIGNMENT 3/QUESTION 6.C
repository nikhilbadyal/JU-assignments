...
if(str_index != 0){
                //printf("In format_doc we're writing :: %s\n",str);
...
for(inti=MAX_WORD_SIZE-1;i>=0; i--){
intcount[37] = {0};
for(int j=0; j<STR_ARRAY_LEN; j++){
            if(isdigit(str[j][i])) count[str[j][i]-'0'+27]++;
            else if(isalpha(str[j][i])) count[str[j][i] - 'a'+1]++;
...
for(int k=0; k<STR_ARRAY_LEN; k++){
            if(isdigit(str[k][i])) strcpy(temp[pos[str[k][i]-'0'+27]++],str[k]);
            else if(isalpha(str[k][i])) strcpy(temp[pos[str[k][i]-'a'+1]++], str[k]);
...
intcomp_strings(char str1[MAX_WORD_SIZE], char str2[MAX_WORD_SIZE]){
for(inti=0; i<MAX_WORD_SIZE-1; i++){
        if(str1[i] == SENTINAL_CHAR || str2[i] == SENTINAL_CHAR){
            if(str2[i]!= SENTINAL_CHAR) return IS_LESSER;
            else if(str1[i] != SENTINAL_CHAR) return IS_GREATER;
            else continue;
        }
...
while(doc_tmp != NULL){
        if(strcmp(doc_tmp->doc_name,doc_name) == IS_EQUAL)
            return;
        if(doc_tmp->next_doc != NULL) doc_tmp = doc_tmp->next_doc;
        else break;
    }
...
if(*head == NULL){
        *head = create_node(str[start_index],doc_name);
        node* curr = *head;
for(inti=start_index+1; i<STR_ARRAY_LEN; i++){
...
curr = curr->next;
        ...
        if(comp_strings(str[start_index],(*head)->word) == IS_LESSER)
insert_front(head,str[start_index++],doc_name);
            node* curr = *head;

for(inti=start_index; i<STR_ARRAY_LEN; i++){
                while(comp_strings(str[i],curr->word) == IS_GREATER){
                    if(curr->next != NULL)
curr = curr->next;
                    else break;
                }
...
while(head!= NULL){
for(inti=0; i<MAX_WORD_SIZE; i++)
            if(head->word[i]!=SENTINAL_CHAR) printf("%c",head->word[i]);
            else printf(" ");
printf("\t");
...
if(comp_strings(formatted_q, prev_req->word) == IS_LESSER){
while(comp_strings(formatted_q, prev_req->word) != IS_EQUAL){
...
printf("Your query cannot be found in DB\n");
    ...
    else if(comp_strings(formatted_q, prev_req->word) == IS_GREATER){
while(comp_strings(formatted_q, prev_req->word) != IS_EQUAL){
